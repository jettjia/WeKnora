                ...
                <t-tooltip :content="item.title" placement="right" :disabled="!uiStore.sidebarCollapsed">
                    <div @click="handleMenuClick(item.path)" @mouseenter="mouseenteMenu(item.path)"
                        ...
                        <div class="menu_item-box">
                            <div class="menu_icon">
                                <img class="icon"
                                    :src="getImgSrc(item.icon == 'zhishiku' ? knowledgeIcon : item.icon == 'agent' ? agentIcon : item.icon == 'automation' ? automationIcon : item.icon == 'organization' ? organizationIcon : item.icon == 'logout' ? logoutIcon : item.icon == 'setting' ? settingIcon : item.icon == 'semantic' ? 'semantic.svg' : prefixIcon)"
                                    alt="">
                            </div>
...
const topMenuItems = computed<MenuItem[]>(() => {
    return (visibleMenuArr.value as unknown as MenuItem[]).filter((item: MenuItem) =>
        item.path === 'knowledge-bases' || item.path === 'agents' || item.path === 'automations' || item.path === 'organizations' || item.path === 'semantic' || item.path === 'creatChat'
    );
});

const bottomMenuItems = computed<MenuItem[]>(() => {
    return (visibleMenuArr.value as unknown as MenuItem[]).filter((item: MenuItem) => {
        if (item.path === 'knowledge-bases' || item.path === 'agents' || item.path === 'automations' || item.path === 'organizations' || item.path === 'semantic' || item.path === 'creatChat') {
            return false;
        }
        return true;
    });
...
let automationIcon = ref('automation.svg');
...
const handleMenuClick = async (path: string) => {
    if (path === 'knowledge-bases') {
        ...
        router.push('/platform/automations')
