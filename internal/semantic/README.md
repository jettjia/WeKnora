...
数据组成员支持跨空间授权: 与本空间同属一个共享空间的空间, 其成员可被加入
本空间的数据组 (保存时按组织关系校验, 组织成员为租户级)。组解析按 user_id
全局匹配, B 空间用户携带 A 空间数据组凭证后, 可访问显式授权给该组的模型与
操作 (accessPolicy 把关)。

**授权回收是手动的**: 用户被移出共享空间 / 删账号时不级联清理数据组成员,
由管理员在数据组界面移除 (group.members 审计可查)。这是有意取舍——查询
路径零开销。
